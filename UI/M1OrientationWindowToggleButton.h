...
public:
    ...
    void internalDraw(Murka & m) {
        ...
        if (!showGimmick) {
            ...
            m.drawCircle(shape.size.x / 2 + cos(degToRad(juce::MathConstants<double>::twoPi * gimmickAngleDegrees - 90)) * (shape.size.x - 5) / 2,
                         shape.size.y / 2 + sin(degToRad(juce::MathConstants<double>::twoPi * gimmickAngleDegrees - 90)) * (shape.size.y - 5) / 2,
                         2);
        }
